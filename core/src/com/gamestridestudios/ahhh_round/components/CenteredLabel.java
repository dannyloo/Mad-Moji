...
public class CenteredLabel extends Label {
    ...
    public CenteredLabel(CharSequence text, LabelStyle style, float fontSize, float screenHeight, float centerAroundX, float centerAroundY) {
        super(text, style);
    ...
    public void setText(CharSequence newText) {
        ...
        float verticalOffset = -fontSize / 4.6875f;
        if (centerAroundY > screenHeight / 2) {
            verticalOffset *= -1;
        }
        setPosition(centerAroundX - (layout.width / 2) + fontSize / 22.5f, centerAroundY - (layout.height / 2) + verticalOffset);
    }
}
