...
import com.qeue.madmoji.events.PurchaseAdRemovalEvent;
...
public class RemoveAdsHandler {
    ...
    public RemoveAdsHandler(Activity activity, Bus bus, GameActivityStore gameActivityStore) {
    ...
    @Subscribe
    public void purchaseAdRemoval(PurchaseAdRemovalEvent event) {
        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                try {
                    SkuDetails removeAdsSku = billingService.queryInventory(true, null, null).getSkuDetails(activity.getResources().getString(com.qeue.madmoji.R.string.remove_ads_sku));
    ...
    private void connectToBilling() {
        billingService = new IabHelper(activity, activity.getResources().getString(com.qeue.madmoji.R.string.public_key));
        billingService.startSetup(new IabHelper.OnIabSetupFinishedListener() {
            @Override
            public void onIabSetupFinished(IabResult result) {
                bus.post(new com.qeue.madmoji.events.UpdateRemoveAdsButtonEvent(result.isSuccess()));
                if (result.isSuccess()) {
    ...
    private void startPurchaseProcess() {
        try {
            billingService.launchPurchaseFlow(activity, activity.getResources().getString(com.qeue.madmoji.R.string.remove_ads_sku), IabHelper.ITEM_TYPE_INAPP, null, 0, new IabHelper.OnIabPurchaseFinishedListener() {
                @Override
    ...
    private void purchaseFinished(IabResult result, Purchase info) {
        if (result.isSuccess() || result.getResponse() == BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED) {
            new AlertDialog.Builder(activity)
                    .setMessage(com.qeue.madmoji.R.string.remove_ads_success_message)
    ...
    private void purchaseSuccess() {
        ...
        bus.post(new com.qeue.madmoji.events.UpdateRemoveAdsButtonEvent(false));
    }

    private void purchaseError() {
        new AlertDialog.Builder(activity)
                .setMessage(com.qeue.madmoji.R.string.remove_ads_error_message)
                .setPositiveButton("Okay", null).show();
    }
            ...
            try {
                billingService.queryInventoryAsync(true, null, null, new IabHelper.QueryInventoryFinishedListener() {
                    ...
                    public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
                        Purchase purchase = inventory.getPurchase(activity.getResources().getString(com.qeue.madmoji.R.string.remove_ads_sku));
