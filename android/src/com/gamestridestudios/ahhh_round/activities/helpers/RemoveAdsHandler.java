...
public class RemoveAdsHandler {
    ...
    public void purchaseAdRemoval(PurchaseAdRemovalEvent event) {
        activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    SkuDetails removeAdsSku = billingService.queryInventory(true, null, null).getSkuDetails(activity.getResources().getString(R.string.remove_ads_sku));
    ...
    private void connectToBilling() {
        billingService = new IabHelper(activity, activity.getResources().getString(R.string.public_key));
    ...
    private void startPurchaseProcess() {
        try {
            billingService.launchPurchaseFlow(activity, activity.getResources().getString(R.string.remove_ads_sku), IabHelper.ITEM_TYPE_INAPP, null, 0, new IabHelper.OnIabPurchaseFinishedListener() {
                @Override
    ...
    private void purchaseFinished(IabResult result, Purchase info) {
        if (result.isSuccess() || result.getResponse() == BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED) {
            new AlertDialog.Builder(activity)
                    .setMessage(R.string.remove_ads_success_message)
    ...
    private void purchaseError() {
        new AlertDialog.Builder(activity)
                .setMessage(R.string.remove_ads_error_message)
                .setPositiveButton("Okay", null).show();
    }
            ...
            try {
                billingService.queryInventoryAsync(true, null, null, new IabHelper.QueryInventoryFinishedListener() {
                    ...
                    public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
                        Purchase purchase = inventory.getPurchase(activity.getResources().getString(R.string.remove_ads_sku));
